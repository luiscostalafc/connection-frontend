...
export default function ImageUpload(props) {
  ...
  return (
    <div style={{marginRight: 50}} className="fileinput text-center">
      <input type="file" onChange={handleImageChange} ref={fileInput} />
      <div className={'thumbnail' + (avatar ? ' img-circle' : '')}>
        <img width="50px" height="70px" src={imagePreviewUrl} alt="..." />
      </div>
      <div>
        {file === null ? (
          <Button color="info" size="sm" {...addButtonProps} onClick={() => handleClick()}>
            {avatar ? 'Addicionar foto' : 'Selecione a imagem'}
          </Button>
        ) : (
          <span>
